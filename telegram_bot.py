...
def read_users():
    ...
    if not os.path.exists(name): return users
    users = set()
    admins = set()
    f = open(name)
    for s in f:
...
def xis_user_ok(bot,update):
    if not is_user_ok(update.message.from_user.first_name,update.message.from_user.last_name):
        bot.send_message(chat_id=update.message.chat_id,text="<i>(.. You are not authorized to use this service.)</i>", parse_mode=ParseMode.HTML)
...
def xis_admin_ok(bot,update):
    if not is_admin_ok(update.message.from_user.first_name,update.message.from_user.last_name):
        bot.send_message(chat_id=update.message.chat_id,text="<i>(.. You are not authorized for this operation.)</i>", parse_mode=ParseMode.HTML)
...
def read_messages():
    ...
    if not os.path.exists(msgname): return []
    #f = file(msgname)
    ...
    for s in alls.split("\n"):
        s += "\n"
        if s.startswith("---"):
            if len(curr_message)==0: continue
...
def hello(bot, update):
    update.message.reply_text(
        'Hello {}'.format(update.message.from_user.first_name) + ", I have %d notes" % (len(messages)+len(new_messages)) )
...
def send_message(bot,update,mess_no,full=False):
    ...
    if len(messages)<mess_no:
        bot.send_message(chat_id=update.message.chat_id,
                         text="<i>(..could not find message %03d)</i> " % (mess_no) , parse_mode=ParseMode.HTML)
    ...
    if (m1.startswith("/d#o#c")):
        ...
        if full:
            try:
                ...
                bot.send_message(chat_id=update.message.chat_id,
                                 text="[%03d] <i>(..document is missing %s)</i> " % (mess_no,fname), parse_mode=ParseMode.HTML)
        else:
        ...
        if full:
            ...
            try:
                ...
                bot.send_message(chat_id=update.message.chat_id,
                                 text="[%03d] <i>(..picture is missing %s)</i> " % (mess_no,m1), parse_mode=ParseMode.HTML)
...
def find_substring(bot,update,args):
    if not xis_user_ok(bot,update): return
    included = []
    excluded = []
    mustbe  = []
    anything = False
    ...
    if len(args)==0:
        update.message.reply_text("Please use syntax:  /f [+<must_be_sub_string1> ..] [!<must_not_be_sub_string1> ..] [<substring1> ..]  ")
        return
    ...
    for m1 in messages:
        ...
        if maybe:
            ...
            if done >=max_messages_in_search:
                bot.send_message(chat_id=update.message.chat_id,
                                 text="<i>(.. there are more notes in search, please rectify.)</i>",parse_mode=ParseMode.HTML)
                break
    ...
    if not anything :
        bot.send_message(chat_id=update.message.chat_id,text="<i>(.. there are no matches.)</i>", parse_mode=ParseMode.HTML)
...
def del_message(bot,update,args):
    if not xis_admin_ok(bot,update): return

    if len(args)<1:
        bot.send_message(chat_id=update.message.chat_id,
                         text="<i>(.. Please specify at least one message number.)</i>", parse_mode=ParseMode.HTML)
    ...
    if repl!="" : bot.send_message(chat_id=update.message.chat_id, text=repl ,parse_mode=ParseMode.HTML)
...
def get_pic(bot,update,args):
    ...
    if ndx>=len(pic_files):
        bot.send_message(chat_id=update.message.chat_id,
                         text="<i>(.. Photo #%d not found.)</i>" %(ndx+1), parse_mode=ParseMode.HTML)
        return
    ...
    try:
        ...
        bot.send_message(chat_id=update.message.chat_id, text="<i>(..picture #%d is missing)</i> " % (ndx+1),parse_mode=ParseMode.HTML)
...
def del_pic(bot,update,args):
    ...
    if len(args)<1:
        bot.send_message(chat_id=update.message.chat_id,
                         text="<i>(.. Please specify at least one picture number.)</i>", parse_mode=ParseMode.HTML)
    ...
    if (pic_files[ndx]=="(..deleted)"):
        bot.send_message(chat_id=update.message.chat_id, text="<i>(..picture #%d is already deleted)</i> " % (ndx+1),parse_mode=ParseMode.HTML)
        return
    ...
    try:
        ...
        bot.send_message(chat_id=update.message.chat_id, text="<i>(..picture #%d deleted)</i> " % (ndx+1),parse_mode=ParseMode.HTML)
...
def find_tag(bot,update,args):
    if not xis_user_ok(bot,update): return
    if len(args)<1:
        bot.send_message(chat_id=update.message.chat_id,text="<i>(.. Please specify tag.)</i>", parse_mode=ParseMode.HTML)
        return
...
def tags_dir(bot,update,args):
    ...
    if len(rep)>0 :  rep = rep[:len(rep)-1]
    bot.send_message(chat_id=update.message.chat_id,
                         text=rep, parse_mode=ParseMode.HTML)
...
def delusettings(bot,update,args):
    ...
    bot.send_message(chat_id=update.message.chat_id,
                 text="<i>(.. session level user settings deleted.)</i>" , parse_mode=ParseMode.HTML)
...
def showsettings(bot,update,args):
    ...
    if ss is None or len(ss)==0: m+="<i>(None)</i>\n"
    else:
        for k in ss.items():
            m+="%s=%s\n" %k
...
def pics_dir(bot,update,args):
    ...
    if len(txt)==0:
        bot.send_message(chat_id=update.message.chat_id,
                         text="<i>(.. No pictures found.)</i>", parse_mode=ParseMode.HTML)
    else:
...
def just_message(bot,update):
    ...
    if txt is not None:
        ...
        save_messages()    ## debug
        bot.send_message(chat_id=update.message.chat_id,
                         text="<i>(.. message saved as [%03d].)</i>" % len(messages), parse_mode=ParseMode.HTML)
    ...
    if ph is not None and len(ph)>0:
        ...
        tst = time.localtime()
        fname = pic_path + "%d-%02d-%02d_%02d%02d%02d" % (tst.tm_year, tst.tm_mon, tst.tm_mday ,tst.tm_hour,tst.tm_min,tst.tm_sec)
        ...
        if doc is not None:
            ...
            if fname is not None:
                ...
                if not upath.endswith("/") : upath+="/"

                fname1 = doc_path+upath+fname
                if not os.path.exists(doc_path+upath):  os.mkdir(doc_path+upath)
...
def techhelp(bot,update,args):
    ...
    bot.send_message(chat_id=update.message.chat_id,
                 text= t, parse_mode=ParseMode.HTML)
